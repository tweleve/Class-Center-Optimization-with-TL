...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs_col, targets_col, inputs_row, targets_row):
        ...
        # split the positive and negative pairs,根据标签信息分割正负样本
        eyes_ = torch.eye(n, dtype=torch.uint8).cuda()
        # 包含正负样本的布尔掩码
        pos_mask = targets_col.expand(targets_row.shape[0], n).t() == targets_row.expand(n, targets_row.shape[0])
        # neg_mask = 1 - pos_mask# 报错了
        # pos_mask[:, :n] = pos_mask[:, :n] - eyes_ ## 报错了
        neg_mask = ~pos_mask  # 修改为这样
        pos_mask[:, :n] = pos_mask & ~eyes_  # 修改为这样
