...
def build_data(cfg, is_train=True):
    ...
    if is_train:
        ...
        if cfg.DATA.SAMPLE == "RandomIdentitySampler":
            sampler = RandomIdentitySampler(dataset=dataset, batch_size=cfg.DATA.TRAIN_BATCHSIZE,
                                            num_instances=cfg.DATA.NUM_INSTANCES, max_iters=cfg.SOLVER.MAX_ITERS, )
            data_loader = DataLoader(dataset, collate_fn=collate_fn,
                                     batch_sampler=sampler, num_workers=cfg.DATA.NUM_WORKERS,
                                     pin_memory=True)
        elif cfg.DATA.SAMPLE == "Random":
            data_loader = DataLoader(dataset, collate_fn=collate_fn,
                                     shuffle=True, batch_size=cfg.DATA.TRAIN_BATCHSIZE,
        ...
        for x in [cfg.DATA.TEST_IMG_SOURCE, cfg.DATA.QUERY_IMG_SOURCE, cfg.DATA.PKUVID_IMG_SOURCE, ]:
            if len(x) != 0:
                dataset = BaseDataSet(x, transforms=transforms, mode=cfg.INPUT.MODE)
                data_loader = DataLoader(dataset, collate_fn=collate_fn,
                                         shuffle=False, batch_size=cfg.DATA.TEST_BATCHSIZE,
